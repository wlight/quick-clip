...
- 条目列表用 `StackPanel` 单列自上而下堆叠（仿 Win11 剪贴板历史的宽卡片）：图片等比缩放限高、文本三行截断，序号 / 类型 / 时间作为一行元信息压在卡片底部；悬停时卡片右上角浮出图钉、二维码、OCR 与「…」菜单（浮层底色跟随卡片状态，压在文字上不糊）。该面板不虚拟化，一次最多渲染 `MainViewModel.MaxRenderedItems = 150` 条，超出部分靠搜索与筛选定位，避免浮出时批量解码缩略图。
...
- `↑ / ↓`：在条目列表中移动光标（单列布局即逐项移动；若换回 `WrapPanel` 网格则由 `MainWindow.GetListColumns` 按实宽算列数，改为整行跨列移动）。
- `← / →`：单列布局下不移动列表（光标焦点在搜索框时归文本框）。仅当面板改为多列网格时，才在同一行内左右移动、行首 / 行尾不折返；搜索框内则需光标已抵文本边界才接管。
- `Enter`：粘贴当前选中项。
